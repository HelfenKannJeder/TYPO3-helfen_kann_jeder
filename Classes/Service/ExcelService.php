...
class Tx_HelfenKannJeder_Service_ExcelService implements t3lib_Singleton {
	...
	private $spreedsheet;
	private $matrix;
	private $activityfieldRepository;
	...
	public function load($document) {
		$this->clear();
		$this->spreedsheet = new Spreadsheet_Excel_Reader($document);
	}
	...
	public function setActivityfieldRepository($activityfieldRepository) {
		$this->activityfieldRepository = $activityfieldRepository;
	}

	public function setActivityRepository($activityRepository) {
	...
	public function toMatrix() {
		...
		$num = 2;
		do {
			$lastContent = $this->spreedsheet->val(1, $num);

			$field = trim($lastContent);
			if (empty($field)) break;

			if ($field{0} == '"') $field = substr($field,1);
			if ($field{strlen($field)-1} == '"') $field = substr($field, 0, strlen($field)-1);
			$field = utf8_encode($field);

			$activityfields = $this->activityfieldRepository->findByName($field);

			if (count($activityfields) == 1) {
				$activity = $activityfields->getFirst();
				$activityfieldsOrdered[$num] = $activity;
			} else {
				$this->error = "Activity field not found: ".$field;
				return false;
			}
			$num++;
		} while (!empty($lastContent));
		$activityFieldCount = $num;

		$num = 2;
		do {
			$field = $this->spreedsheet->val($num, 1);
			...
			$field = trim($field);
			$field = utf8_encode($field);
			if ($field == "Personen aus Gebäuden retten") $field = "aus Gebäuden retten";
			if ($field == "Personen aus Höhen und Tiefen retten") $field = "aus Höhen und Tiefen retten";
			if ($field == "Personen aus unwegsamem Gelände retten") $field = "aus unwegsamem Gelände retten";
			if ($field == "organisieren / koordinieren / verwalten") $field = "koordinieren / verwalten";
			if ($field == "Personen aus Fahrzeugen retten und befreien") $field = "aus Verkehrsmitteln retten und befreien";
			if ($field == "kochen und verpflegen") $field = "kochen, verpflegen, bewirten";
			if ($field == "Mannschaftssport betreiben") $field = "Sport machen";
			if ($field == "sprengen (Gebäude, ...)") $field = "sprengen";
			if ($field == "mit meinen Klassenkameraden Action haben") $field = "bei einer Jugendgruppe mitmachen";
			if ($field == "hilfebedürftige Personen betreuen") $field = "Menschen betreuen";
			if ($field == "Umwelt-/Naturschutz") $field = "Naturschutz";

			$activities = $this->activityRepository->findByName($field);
			if (count($activities) == 1) {
				$activity = $activities->getFirst();
				for ($num2 = 2; $num2 <= $activityFieldCount; $num2++) {
				...
				$this->error = "Activity not found: ".$field;
				return false;
			}
			$num++;
		} while (!empty($field));
